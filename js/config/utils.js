...
export function debounce(func, delay = 300) {
    ...
    return (...args) => {
        logEvent("test_start", `Déclenchement de debounce, remise à zéro du délai (${delay}ms)`);

        // Annule le précédent appel si la fonction est rappelée trop vite
        clearTimeout(timeout);

        // Définit un nouveau délai avant d'exécuter la fonction
        timeout = setTimeout(() => {
            logEvent("test_end", `Exécution de la fonction après délai (${delay}ms)`);
            func(...args);
        }, delay);
    };
}
...
============================================================================= */
/**
 * Sécurise une chaîne de texte en remplaçant les caractères spéciaux par leur équivalent HTML.
 * 
 * - Protège contre les attaques XSS.
 * - Convertit les caractères spéciaux en entités HTML sécurisées.
 *
 * @param {string} text - Chaîne de texte à sécuriser.
 * @returns {string} Chaîne sécurisée avec les caractères spéciaux encodés.
 */
export function sanitizeText(text) {
    // Vérifie si l'entrée est bien une chaîne de caractères
    if (typeof text !== "string") {
        logEvent("warn", "sanitizeText : Entrée invalide, renvoi d'une chaîne vide.", { text });
        return "";
    }

    logEvent("info", `sanitizeText : Traitement de la chaîne "${text}"`);
    ...
    // Remplace chaque caractère spécial par son équivalent sécurisé
    const sanitizedText = text.replace(/[&<>"'/]/g, (char) => charMap[char] || char);

    logEvent("success", `sanitizeText : Texte sécurisé obtenu -> "${sanitizedText}"`);
